...
function parseBaseHref(argumentsToParse) {
  ...
  if (
    ...
    argumentsToParse[0].startsWith("--base-href=")
  ) {
...
if (process.exitCode) {
  ...
  if (importExitCode !== 0) {
    console.error(
      "Error: Angular build was not started because data import failed.",
    );
    ...
    if (!existsSync(angularCli)) {
      ...
      if (buildResult.error) {
        console.error(
          `Error: could not start Angular build: ${buildResult.error.message}`,
        );
        process.exitCode = 1;
      } else {
        process.exitCode = Number.isInteger(buildResult.status)
          ? buildResult.status
          : 1;
      }
    }

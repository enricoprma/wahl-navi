...
function getWindowsPythonCandidates() {
  ...
  try {
    return readdirSync(installRoot)
      ...
      .sort(
        (a, b) =>
          Number(b.slice("Python".length)) - Number(a.slice("Python".length)),
      )
      .map((directory) => ({
...
export function findPython() {
  for (const candidate of pythonCandidates) {
    const version = getPythonVersion(candidate.command, candidate.arguments);
...
function isSupportedPython(version) {
  return (
    version.major > MIN_PYTHON_VERSION.major ||
    (version.major === MIN_PYTHON_VERSION.major &&
      version.minor >= MIN_PYTHON_VERSION.minor)
  );
}

function getPythonVersion(command, commandArguments) {
  const result = spawnSync(command, [...commandArguments, "--version"], {
    encoding: "utf8",
  });
...
export function runPython(argumentsToRun, options = {}) {
  ...
  if (!python) {
    console.error(
      `Error: Python ${MIN_PYTHON_VERSION.major}.${MIN_PYTHON_VERSION.minor} or newer was not found.`,
    );
    return 1;
  }

  console.log(
    `Using Python ${python.version.major}.${python.version.minor}.${python.version.patch}: ${python.command}`,
  );

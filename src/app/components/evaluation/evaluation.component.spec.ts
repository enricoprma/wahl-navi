...
describe('EvaluationComponent', () => {
  ...
  it('restores results and persists answer/weight edits without changing the resume position', fakeAsync(() => {
    ...
    expect(fixture.nativeElement.querySelector('app-agreement h2').textContent).toContain('67.0%');

    component.changeVote(10, -1);
    render();
    expect(component.agreements[0].party.id).toBe('second');
    expect(component.agreements[0].percent).toBe(100);
    expect(fixture.nativeElement.querySelector('app-agreement h2').textContent).toContain('100.0%');
    expect(JSON.parse(storage.get(KEY)!).votes[0]).toEqual({ statementId: 10, value: -1, weight: 2 });

    component.changeVote(10, 1);
    component.changeVoteWeight(42, 2);
    render();
    expect(component.agreements.map(result => result.percent)).toEqual([50, 50]);
    expect(fixture.nativeElement.querySelector('app-agreement h2').textContent).toContain('50.0%');
  ...
  it('renders unanswered and explicitly skipped statements as no answer and lets a draft weight be used on a new answer', fakeAsync(() => {
    storage.set(KEY, JSON.stringify(savedState({
      currentStatementId: 99, votes: [{ statementId: 10, value: null, weight: 1 }],
    })));
    render();
    expect(component.getVote(42)).toBeUndefined();
    expect(component.agreements.map(result => result.percent)).toEqual([0, 0]);
    ...
    expect(component.getVote(42)).toBeUndefined();
    expect(JSON.parse(storage.get(KEY)!).draftWeights).toEqual([{ statementId: 42, weight: 2 }]);
    component.changeVote(42, 1);
    expect(component.getVote(42)).toEqual({ statementId: 42, value: 1, weight: 2 });
    expect(state.currentStatementId()).toBe(99);
    render();
  }));
});

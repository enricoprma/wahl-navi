...
describe('StartComponent', () => {
  ...
  it('shows Start for an untouched first statement', async () => {
    ...
    await render();
    const labels = [...fixture.nativeElement.querySelectorAll('button')].map((button: any) => button.textContent.trim());
  ...
  it('shows Continue for navigation-only progress and preserves it when continuing or canceling restart', async () => {
    ...
    const open = spyOn(fixture.debugElement.injector.get(MatDialog), 'open')
      .and.returnValue({ afterClosed: () => of(false) } as any);
  ...
  it('offers Continue for a saved draft and removes that progress on confirmed restart', async () => {
    storage.set(KEY, JSON.stringify(savedState({ draftWeights: [{ statementId: 10, weight: 2 }] })));
    await render();

...
describe('MatchingService', () => {
  ...
  const parties: Party[] = [
    { id: 'early-birds', name: 'Early Bird Party', shortName: 'EBP', color: '#F59E0B', description: '' },
    { id: 'night-owls', name: 'Night Owl Alliance', shortName: 'NOA', color: '#4338CA', description: '' },
  ];
  const positions: Position[] = [
    { partyId: 'early-birds', statementId: 1, opinion: 1, justification: null },
    { partyId: 'early-birds', statementId: 2, opinion: -1, justification: null },
  ...
  it('applies double weighting before calculating the percentage', () => {
    ...
    expect(service.calculateAgreements(votes, parties, positions)[1].percent).toBe(33);
  });

  it('returns zero safely when every vote is unanswered', () => {
    const votes: Vote[] = [{ statementId: 1, value: null, weight: 2 }];

    expect(service.calculateAgreements(votes, parties, positions).map(result => result.percent)).toEqual([0, 0]);
  });

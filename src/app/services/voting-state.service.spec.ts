...
describe("VotingStateService", () => {
  ...
  it("migrates valid version-1 progress without losing answers or the resume position", async () => {
    const { draftWeights, ...oldState } = savedState({
      currentStatementId: 42,
      votes: [{ statementId: 10, value: null, weight: 2 }],
    });
  ...
  it("persists draft weights, restores them, and consumes them only when answered", async () => {
    ...
    expect(restored.votes()).toEqual([
      { statementId: 42, value: null, weight: 2 },
    ]);
  ...
  const invalidRecords: [string, () => string][] = [
    ...
    [
      "unsupported schema",
      () => JSON.stringify({ ...savedState(), schemaVersion: 3 }),
    ],
    [
      "mismatched dataset",
      () => JSON.stringify(savedState({ datasetId: "other" })),
    ],
    [
      "unknown current statement",
      () => JSON.stringify(savedState({ currentStatementId: 100 })),
    ],
    [
      "invalid timestamp",
      () => JSON.stringify(savedState({ updatedAt: "invalid" })),
    ],
    [
      "missing draft weights",
      () => JSON.stringify({ ...savedState(), draftWeights: undefined }),
    ],
    [
      "invalid opinion",
      () =>
        JSON.stringify({
          ...savedState(),
          votes: [{ statementId: 10, value: 3, weight: 1 }],
        }),
    ],
    [
      "invalid vote weight",
      () =>
        JSON.stringify({
          ...savedState(),
          votes: [{ statementId: 10, value: 1, weight: 3 }],
        }),
    ],
    [
      "unknown vote ID",
      () =>
        JSON.stringify(
          savedState({ votes: [{ statementId: 100, value: 1, weight: 1 }] }),
        ),
    ],
    [
      "duplicate votes",
      () =>
        JSON.stringify(
          savedState({
            votes: Array(2).fill({ statementId: 10, value: 1, weight: 1 }),
          }),
        ),
    ],
    [
      ...
      () =>
        JSON.stringify({
          ...savedState(),
          draftWeights: [{ statementId: 42, weight: 3 }],
        }),
    ],
    [
      "unknown draft ID",
      () =>
        JSON.stringify(
          savedState({ draftWeights: [{ statementId: 100, weight: 2 }] }),
        ),
    ],
    [
      "duplicate drafts",
      () =>
        JSON.stringify(
          savedState({
            draftWeights: Array(2).fill({ statementId: 42, weight: 2 }),
          }),
        ),
    ],
    [
      "a draft overlapping a vote",
      () =>
        JSON.stringify(
          savedState({
            votes: [{ statementId: 10, value: 0, weight: 1 }],
            draftWeights: [{ statementId: 10, weight: 2 }],
          }),
        ),
    ],
  ];
